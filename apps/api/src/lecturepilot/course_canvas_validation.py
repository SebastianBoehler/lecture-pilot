...
def validate_planned_document(document: CanvasDocument, source_document: CanvasDocument) -> None:
    ...
    thin_sections = [section.title for section in document.sections if _teaching_units(section) < 2]
    if thin_sections:
        names = ", ".join(thin_sections[:3])
        raise ProviderConfigurationError(f"Canvas sections need at least 2 teaching blocks: {names}.")
